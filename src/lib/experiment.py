...
def _check_exp(exp: str | Path) -> Path:
    ...
    if not exp.is_dir():
        raise RuntimeError(
            ...
            f'{f". Try passing {exp.parent} instead" if exp.name == "config.json" else ""}'  # noqa: E501
        )
    return exp
...
def load_config(exp: str | Path) -> JSONDict:
    exp = _check_exp(exp)
    with open(get_config_path(exp)) as f:
        return json.load(f)
...
def dump_config(exp: str | Path, config: JSONDict) -> None:
    exp = _check_exp(exp)
    with open(get_config_path(exp), 'w') as f:
...
def finish(exp: str | Path, report: Report) -> None:
    ...
    print()
    print(utils.add_frame(load_summary(exp)))
    backup(exp)
...
def _check_config_dataclass(cls, key: tuple[str, ...]):
    ...
    # The following check is very incomplete, because it does not cover complex
    # type annotations with dataclasses inside them.
...
def create_report(function: MainFunction, *, add_gpu_info: bool) -> Report:
    report: Report = {'function': utils.get_function_full_name(function)}
...
def _get_config[T](config_type: type[T], config: None | T, exp: str | Path) -> T:
    ...
    if config is None:
        ...
        if utils.is_typed_dict(config_type):
            config = utils.check_typed_dict(config_type, loaded_config)
        elif config_type_is_dataclass:
            config = utils.dataclass_from_dict(config_type, loaded_config)
        else:
            raise ValueError(
...
def run[T](
    ...
    *,
    ...
    function_full_name = (
        None if function is None else utils.get_function_full_name(function)
    )
    utils.print_sep()
    print(
        f'{"" if function_full_name is None else f"{function_full_name} | "}'
        f'{utils.try_get_relative_path(exp)}'
    ...
    print('\nConfig' if utils.is_typed_dict(config_type) else '')
        ...
        if (key == 'time' or key.endswith('_time')) and isinstance(value, float):
            ...
            if 'metrics' in reports[0]:
                ...
                for part in reports[0]['metrics']:
                    ...
                    part_score_std = (
                        float('inf')
                        if any(map(math.isinf, part_scores))
                        else statistics.stdev(part_scores)
...
# >>> Backup (this section is used by the internal infrastructure and can be ignored)


_LAST_SNAPSHOT_TIME = None
_SNAPSHOT_FREQUENCY_SECONDS = 600


def _try_add_to(
    exp: str | Path, other_project_dir: None | str | Path, **copytree_kwargs
) -> None:
    """A function for the internal infrastructure, ignore it."""
    exp = _check_exp(exp)
    if env.get_project_dir() not in exp.parents:
        return
    if other_project_dir is None:
        return
    duplicate(exp, other_project_dir, force=True, **copytree_kwargs)


def add_to_tmp_output(exp: str | Path, **copytree_kwargs) -> None:
    """A function for the internal infrastructure, ignore it."""
    _try_add_to(exp, env.get_tmp_output_dir(), **copytree_kwargs)


def add_to_snapshot(exp: str | Path, **copytree_kwargs) -> None:
    """A function for the internal infrastructure, ignore it."""
    _try_add_to(exp, env.get_snapshot_dir(), **copytree_kwargs)

    if not env.is_local():
        global _LAST_SNAPSHOT_TIME
        if (
            _LAST_SNAPSHOT_TIME is None
            or time.time() - _LAST_SNAPSHOT_TIME > _SNAPSHOT_FREQUENCY_SECONDS
        ):
            import nirvana_dl.snapshot  # type: ignore

            nirvana_dl.snapshot.dump_snapshot()
            _LAST_SNAPSHOT_TIME = time.time()
            logger.info('The snapshot was updated')


def backup(exp: str | Path, **copytree_kwargs) -> None:
    """A function for the internal infrastructure, ignore it."""
    # Do not backup experiments located in the temporary directory.
    if Path(tempfile.gettempdir()).resolve() in _check_exp(exp).resolve().parents:
        return

    add_to_tmp_output(exp, **copytree_kwargs)
    add_to_snapshot(exp, **copytree_kwargs)
